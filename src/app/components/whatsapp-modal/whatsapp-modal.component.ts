    ...
    @if (isOpen) {
      ...
      >
        <div
          ...
          class="relative w-full max-w-lg max-h-[calc(100dvh-1.5rem)] overflow-y-auto bg-[#FAF8F3] rounded-3xl sm:rounded-[32px] border-2 border-[#AFCFC0] shadow-2xl"
          (click)="$event.stopPropagation()"
        >
          <!-- Header -->
          <div class="bg-[#155E5B] p-4 sm:p-6 text-white flex items-center justify-between border-b border-[#0E4543]">
            <div class="flex items-center gap-2.5 sm:gap-3">
              <div class="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-[#0E4543] border border-[#B89452]/60 flex items-center justify-center text-white font-bold text-xs tracking-wider shadow-xs overflow-hidden shrink-0">
                <img src="pehnava-logo.png" alt="Pehnava" class="w-full h-full object-contain" />
              </div>
              <div>
                <div class="flex items-center gap-1.5">
                  <h3 id="whatsapp-modal-title" class="font-bold text-xs sm:text-base text-[#FAF8F3]">Pehnava Boutique</h3>
                  <app-icon name="sparkles" [size]="13" customClass="text-[#D4B270]"></app-icon>
                </div>
                <p class="text-[11px] sm:text-xs text-[#AFCFC0]">{{ whatsappDisplayNumber }} • Ajmer Store</p>
              </div>
            </div>
            ...
            <button
              ...
              class="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-[#0E4543] text-white hover:bg-[#092B2A] flex items-center justify-center transition-colors cursor-pointer shrink-0"
            >
          ...
          <div class="p-4 sm:p-7">
            <p class="text-xs sm:text-sm text-[#71847B] font-normal leading-relaxed mb-4">
            ...
            <form (submit)="handleSubmit($event)" class="space-y-4">
              <div>
                ...
                <textarea
                  ...
                  name="message"
                  rows="3"
                  class="w-full text-xs sm:text-sm p-3 sm:p-3.5 rounded-2xl bg-white border border-[#D5D8D3] focus:outline-hidden focus:border-[#155E5B] text-[#26332F] leading-relaxed shadow-inner"
              ...
              <div class="flex flex-wrap items-center justify-between gap-2 text-xs text-[#71847B] bg-[#F3EEE4] p-3 rounded-xl border border-[#D5D8D3]">
                <span class="flex items-center gap-1.5">
                  <app-icon name="store" [size]="13" customClass="text-[#B8875A] shrink-0"></app-icon>
                  <span>Mayo Link Road, Ajmer</span>
                </span>
                <span class="flex items-center gap-1.5">
                  <app-icon name="clock" [size]="13" customClass="text-[#155E5B] shrink-0"></app-icon>
              ...
              <div class="pt-2">
                <button
                  ...
                  class="w-full btn-pehnava-primary inline-flex items-center justify-center gap-2 py-3.5 rounded-full text-xs sm:text-sm font-semibold tracking-wide cursor-pointer shadow-md active:scale-98 text-center"
                >
